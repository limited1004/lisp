...
![](https://github.com/limited1004/PureScript/blob/master/img/img.png)
...
![](https://github.com/limited1004/PureScript/blob/master/img/img2.png)

Здесь, когда мы используем функцию max для 2 и 3, функция возвращает 3. Это совершенно очевидно.

Итак, давайте проверим, какой тип макс. PSCi возвращается
...
Давайте разберем это на части и выясним, что это на самом деле представляет.
1.  `forall a. : This is similar to the following mathametical notation`
    
   `  for all x, x′∈X`

2. Ord - это класс типов, определенный в модуле Prelude. Класс типов Ord представляет типы, которые поддерживают сравнения с общим порядком. Он должен соответствовать законам отражения, транзитивности и антисимметрии (будут рассмотрены в следующих статьях).

 `a -> a -> a`

3. Это самая важная часть. Это говорит о том, что он принимает два входа типа a и типа a и возвращает значение типа a.

макс 2 3 принимает 2 и 3 в качестве входов. 2 и 3 имеют один и тот же тип данных - Int, поэтому он возвращает значение 3 того же типа данных Int.

Здесь не указывается конкретный тип данных. Таким образом, мы можем применить эту функцию и к другим типам данных.

Когда мы применяем функцию max к Char

`> max ‘a’ ‘b’`

Оба входа имеют одинаковые типы, так что это удовлетворяет максимальному определению.

Давайте рассмотрим другую функцию, минусы (Примечание: минусы в нижнем регистре «c»).

Если мы хотим знать, как работать с функцией, давайте применим команду: type к cons.

Параметр cons определен в модуле Data.Array, поэтому мы должны сначала импортировать модуль, прежде чем продолжить.

![](https://github.com/limited1004/PureScript/blob/master/img/img3.png)

Мы получаем результат:

`forall a. a-> Array a -> Array a`

На данный момент, давайте рассмотрим блок только после a.

`a-> Array a -> Array a`

Это утверждение говорит, что функция cons принимает 2 входа и возвращает 1 выход.

Input 1 : Элемент типа a, (eg. Int)

Input 2: Массив элементов типа a, (eg. Array Int)

Output: Массив элементов типа a (eg. Array Int)

Что именно делает эта функция?

Глядя на документацию, мы можем понять, что

cons Прикрепляет элемент к началу массива, создавая новый массив.
